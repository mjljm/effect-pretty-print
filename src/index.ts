...
import { HashSet, Match, MutableHashMap, Option, ReadonlyArray, Tuple, pipe } from 'effect';
export {
	ansiFormatter,
	ansiKeyFormatter,
	basicFormatter,
	basicKeyFormatter,
	ansi as defaultOptionsWithAnsiColors,
	type Options
} from '@mjljm/effect-pretty-print/Options';

export const _ = Options._;
...
export const stringify = (u: unknown, options?: Options.Options) =>
	pipe(
		...
		(defaultOptions) => Options.makeAllRequired({ ...defaultOptions, ...options }),
		// Add Precalcs
		...
		(optionsWithDefaultsAndPrecalcs) =>
			internalToString(
				...
				MutableHashMap.empty<MFunction.RecordOrArray, FormattedString.FormattedString>(),
...
const internalToString = (
	...
	pipe(
		MFunction.isRecordOrArray(u) ? MutableHashMap.get(stringified, u) : Option.none(),
		...
		Option.getOrElse(() =>
			pipe(
				...
				Match.when(MMatch.recordOrArray, (obj) =>
					handleArrayOrObject(obj, stringified, parents, options, currentTab, depth)
				),
				Match.exhaustive
			)(u)
		),
		(value) =>
			MFunction.isRecordOrArray(u) ? (MutableHashMap.set(stringified, u, value), value) : value
	);
						...
						pipe(
							...
							ReadonlyArray.mapAccum(0, (acc, property) =>
								pipe(
									...
									(s) =>
										Tuple.make(
											acc + property.prefixedKey.printedLength + split.sepsLength + s.printedLength,
											s
										),
									(z) => z
								)
							...
							([length, values]) =>
								length - sepLength + split.marksLength <= options.noLineBreakIfShorterThan
									? SplitObject.stringify(split, values, _(''), currentTab, _(''), depth)
